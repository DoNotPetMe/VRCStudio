          ...
          {active === 'appearance' && (
            <>
              ...
              <Section title="Audio Visualizer" icon={Volume2}>
                <p className="text-xs text-surface-500">Animated background reacting to system audio. Captures audio when permitted; falls back to procedural animation otherwise.</p>
                <Toggle label="Enable visualizer" description="Renders an animated equaliser behind the app" checked={theme.visualizer.enabled} onChange={v => setVisualizer({ enabled: v })} />
                <div className={theme.visualizer.enabled ? '' : 'opacity-50 pointer-events-none'}>
                  <label className="block text-sm font-medium mb-2">Style</label>
                  <OptionRow options={['bars', 'blocks', 'wave', 'radial', 'dots']} labels={{ bars: 'Bars', blocks: 'Blocks', wave: 'Wave', radial: 'Radial', dots: 'Dots' }} value={theme.visualizer.style ?? 'bars'} onChange={v => setVisualizer({ style: v as 'bars' | 'blocks' | 'wave' | 'radial' | 'dots' })} />
                  <label className="block text-sm font-medium mt-4 mb-2">Frequency Focus</label>
                  <OptionRow options={['all', 'bass', 'mids', 'treble']} value={theme.visualizer.focus} onChange={v => setVisualizer({ focus: v as 'all' | 'bass' | 'mids' | 'treble' })} />
          ...
          {active === 'general' && (
            <>
              <Section title="Window Behavior" icon={SettingsIcon}>
                <Toggle label="Start Minimized" description="Start VRC Studio minimized to the system tray" checked={settings.general.startMinimized} onChange={v => updateGeneral({ startMinimized: v })} />
                <Toggle label="Minimize to Tray" description="Send to system tray instead of closing when you click ✕" checked={settings.general.minimizeToTray} onChange={v => { updateGeneral({ minimizeToTray: v }); window.electronAPI?.setMinimizeToTray(v); }} />
                <Toggle label="Always on Top" description="Keep the window pinned above all other applications" checked={settings.general.alwaysOnTop} onChange={v => { updateGeneral({ alwaysOnTop: v }); window.electronAPI?.setAlwaysOnTop(v); }} disabled={!window.electronAPI} />
                <Toggle label="Confirm Before Closing" description="Ask for confirmation before quitting the app" checked={settings.general.confirmClose} onChange={v => updateGeneral({ confirmClose: v })} />
                <Toggle label="Launch on System Startup" description="Start VRC Studio automatically when your computer boots" checked={autoLaunch} onChange={handleAutoLaunch} disabled={!window.electronAPI} />
                {!window.electronAPI && <p className="text-xs text-amber-400">Auto-launch requires the desktop (Electron) build.</p>}
              </Section>
          ...
          {active === 'about' && (
            <Section title="About VRC Studio" icon={Info}>
              ...
              <div className="pt-2 text-center">
                <p className="text-[11px] text-surface-600">Made by DoNotPetMe</p>
              </div>
            </Section>
          )}
...
function OptionRow({ options, labels, value, onChange }: { options: string[]; labels?: Record<string, string>; value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex gap-2 flex-wrap">
      {options.map(opt => (
        <button key={opt} onClick={() => onChange(opt)} className={`px-4 py-1.5 rounded-lg text-sm border transition-colors capitalize ${value === opt ? 'border-accent-500 bg-accent-500/15 text-accent-400' : 'border-surface-700 bg-surface-800 text-surface-400 hover:border-surface-600'}`}>
          {labels?.[opt] ?? opt}
        </button>
      ))}
    </div>
